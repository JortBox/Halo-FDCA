...
parser.add_argument('d_path',help='(str) Path to clusters radio FITS image.', type=str)
...
parser.add_argument('-m_path',help='(str) Path to clusters .reg mask file. Default: None', default=None, type=str)
...
parser.add_argument('-s',help='(bool) Whether to save the mcmc sampler chain in a fits file. When true, k will be included as parameter. Default: True.',default=True, type=str2bool)
parser.add_argument('-run',help='(bool) Whether to run a MCMC routine or skip it to go straight to processing. can be done if a runned sample already exists in the output path. Default: True',default=True, type=str2bool)
...
def init_logging(args):
    ...
    d = {
            ...
            'file': {
                ...
                'filename': path+'log/'+args.object+' '+now+'.log',
                'mode': 'w',
                'formatter': 'detailed',
            },
            },
...
halo = FDCA.Radio_Halo(args.object, args.d_path, maskpath=args.m_path, mask=args.m,
...
if args.run:
    fit  = FDCA.markov_chain_monte_carlo.fitting(halo, halo.data_mcmc, args.model, p0,
                                                ...
                                                mask=args.m, maskpath=args.m_path,
